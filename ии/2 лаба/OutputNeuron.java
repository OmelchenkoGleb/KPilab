...
public class OutputNeuron implements ActivationFunction, Neuron {
        ...
        public void forwardSignalReceived(Neuron from, Double value) {
            ...
            if (backwardConnections.size() == signalReceived){
                Sum = backwardConnections.entrySet().stream().mapToDouble(connection -> inputSignals.get(connection.getKey()) * connection.getValue()).sum();
                forwardResult = ActivationFunction(Sum);
            }
        }
        @Override
